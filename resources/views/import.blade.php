...
@section('content')
    <div class="space-y-6">
        ...
        <form x-data="importForm()" x-on:submit.prevent="startLoading" action="{{ route('import.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="space-y-2">
                <label for="files" class="text-sm font-medium text-slate-800">Arquivos .DEC</label>
                <input id="files" name="files[]" type="file" accept=".dec,text/plain" multiple
                ...
                <p class="text-xs text-slate-500">Envie um ou mais arquivos .DEC (máx 5MB cada).</p>
                @error('files')
            ...
            <button type="submit"
                    :disabled="loading"
                    class="inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-slate-800 disabled:opacity-60">
            ...
            return {
                ...
                async startLoading() {
                    ...
                    if (!input || !input.files || input.files.length === 0) {
                        alert('Selecione pelo menos um arquivo .DEC.');
                        return;
                    }
                    this.loading = true;
                    this.progress = 0;
                ...
                uploadWithProgress(fd, totalFiles) {
